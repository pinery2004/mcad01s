// Suudoku.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには Suudoku.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE SuudokuDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Suudoku.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(SuudokuDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(SuudokuDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Suudoku.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(SuudokuDLL);
	}
	return 1;   // OK
}

// MCmd7.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには MCmd7.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MCmd7DLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MCmd7.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(MCmd7DLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MCmd7DLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MCmd7.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MCmd7DLL);
	}
	return 1;   // OK
}

// MCmd3.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには MCmd3.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MCmd3DLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MCmd3.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(MCmd3DLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MCmd3DLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MCmd3.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MCmd3DLL);
	}
	return 1;   // OK
}

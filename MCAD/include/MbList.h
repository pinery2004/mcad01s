...
public:
	...
	CNode*		m_pNodeFree;						// 開放ノードの基点
	MINT		m_nRecSize;							// レコードサイズ( byte)

	MbPlex*		m_pBlocks;						// ノード確保領域
...
// Operations
	// 列の先頭に実体を入れる						pEnt	: 挿入する実体
	MPOSITION	Push( MOBJECT*  pEnt);
	MPOSITION 	AddHead( MOBJECT*  pEnt)
						{ return Push( pEnt);}
	// 列の先頭の実体を取り出しノードを消去する		return	: 0: 実体無し　≠0:実体のポインタ
	MOBJECT* Pop( );
	MOBJECT* RemoveHead( )
						{ return Pop( );}

	// 列の末尾に実体を入れる						pEnt	: 挿入する実体
	MPOSITION	Inject( MOBJECT*  pEnt);
	MPOSITION 	AddTail( MOBJECT*  pEnt)
						{ return Inject( pEnt);}
	// 列の末尾の実体を取り出しノードを消去する		return	: 0: 実体無し　≠0:実体のポインタ
	MOBJECT*	Eject( );
	MOBJECT*	RemoveTail( )
						{ return Eject( );}
	...
	// ノード位置を進め、次の実体を得る					return: 実体、最終より後なら０
	...
	// ノード位置を戻し、手前の実体を得る				return: 実体、先頭より前なら０
...
// Operations
	...
	MPOSITION 	AddHead( MOBJECT*  pEnt)
						{ return Push( pEnt);}
	// 列の先頭の実体を取り出し消去する				return	: 0: 実体無し　1:実体有り
	...
	MPOSITION 	AddTail( MOBJECT*  pEnt)
						{ return Inject( pEnt);}
	// 列の末尾の実体を取り出し消去する				return	: 0: 実体無し　1:実体有り

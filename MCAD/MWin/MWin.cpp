// MWin.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには MWin.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MWinDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MWin.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(MWinDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MWinDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MWin.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MWinDLL);
	}
	return 1;   // OK
}

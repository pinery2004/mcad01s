...
#ifdef _MANAGED
#error /clr と共にコンパイルするには MGeo.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MGeoDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MGeo.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(MGeoDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MGeoDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MGeo.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MGeoDLL);
	}
	return 1;   // OK
}

// Md.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには Md.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MdDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("Md.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if (!AfxInitExtensionModule(MdDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MdDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("Md.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MdDLL);
	}
	return 1;   // OK
}

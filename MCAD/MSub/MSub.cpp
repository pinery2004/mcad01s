// MSub.cpp : DLL の初期化ルーチンです。
//

#include "stdafx.h"
#include <afxdllx.h>
#ifdef _MANAGED
#error /clr と共にコンパイルするには MSub.cpp 内の操作手順を参照してください。
...
static AFX_EXTENSION_MODULE MSubDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("MSub.DLL Initializing!\n");
		
		// 拡張 DLL を 1 回だけ初期化します。
		if ( !AfxInitExtensionModule(MSubDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(MSubDLL);

	}
	else if ( dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("MSub.DLL Terminating!\n");

		// デストラクタが呼び出される前にライブラリを終了します
		AfxTermExtensionModule(MSubDLL);
	}
	return 1;   // OK
}

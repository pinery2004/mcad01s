...
#include "stdafx.h"
#include "MemDC.h"
#define DLL_NO_IMPORT_GRIDCTRL_DO
#define DLL_EXPORT_GRIDCTRL_DO
#include "GridCtrl.h"
#include "MgDefine.h"
...
BOOL CGridCtrl::Create(const RECT& rect, CWnd* pParentWnd, UINT nID, DWORD dwStyle)
{
    ...
    for ( i = 0; i < m_nCols; i++)
        m_arColWidths[i] = m_cellDefault.GetMbWidth();
...
{
    if ( !IsValid(m_idCurCell))
    {
        CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
        return;
    }

    CCellID next = m_idCurCell;
    ...
    {
        OnEditCell(m_idCurCell.row, m_idCurCell.col, CPoint( -1, -1), VK_LBUTTON);
    }
    else
    {
        CWnd::OnKeyDown(nChar, nRepCnt, nFlags);
...
void CGridCtrl::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if ( !IsCTRLpressed() && m_MouseMode == MOUSE_NOTHING && nChar != VK_ESCAPE)
    {
        if ( !m_bHandleTabKey || (m_bHandleTabKey && nChar != VK_TAB))
            OnEditCell(m_idCurCell.row, m_idCurCell.col, CPoint( -1, -1), nChar);
    }
...
LRESULT CGridCtrl::OnImeChar(WPARAM wCharCode, LPARAM)
{
    ...
    if ( !IsCTRLpressed() && m_MouseMode == MOUSE_NOTHING && wCharCode != VK_ESCAPE) 
        OnEditCell(m_idCurCell.row, m_idCurCell.col, CPoint( -1, -1), (UINT)wCharCode);
    return 0;
}
...
void CGridCtrl::OnEndInPlaceEdit(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    case VK_END:
        ...
        OnEditCell(m_idCurCell.row, m_idCurCell.col, CPoint( -1, -1), (UINT)pgvItem->lParam);
    }
...
CCellID CGridCtrl::SetFocusCell(CCellID cell)
{
    ...
    if ( IsValid( idPrev))
    {
        ...
        if ( GetTrackFocusCell() && idPrev.col != m_idCurCell.col)
            for ( int row = 0; row < m_nFixedRows; row++)
                RedrawCell(row, idPrev.col);
        if ( GetTrackFocusCell() && idPrev.row != m_idCurCell.row)
    ...
    if ( IsValid(m_idCurCell))
    {
        SetItemState(m_idCurCell.row, m_idCurCell.col,
            GetItemState(m_idCurCell.row, m_idCurCell.col) | GVIS_FOCUSED);

        RedrawCell(m_idCurCell); // comment to reduce flicker

        if ( GetTrackFocusCell() && idPrev.col != m_idCurCell.col)
            for ( int row = 0; row < m_nFixedRows; row++)
                RedrawCell(row, m_idCurCell.col);
        if ( GetTrackFocusCell() && idPrev.row != m_idCurCell.row)
            for ( int col = 0; col < m_nFixedCols; col++)
                RedrawCell(m_idCurCell.row, col);
                              ...
                              BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    ...
    //if ( currentCell.col == m_idCurCell.col) return;
                           ...
                           BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    ...
    //if ( currentCell.row; == m_idCurCell.row) return;
                            ...
                            BOOL bForceRedraw /*=FALSE*/, BOOL bSelectCells /*=TRUE*/)
{
    ...
    //else if ( currentCell == m_idCurCell) return;
...
void CGridCtrl::ValidateAndModifyCellContents( int nRow, int nCol, LPCTSTR strText)
{
    ...
    {
        CString strCurText = GetItemText(nRow, nCol);
        if ( strCurText != strText)
        {
            ...
            {
                SetItemText(nRow, nCol, strCurText);
            }
        }
    }
}
...
void CGridCtrl::OnEditSelectAll()
{
    ...
    SelectAllCells();
    SendMessageToParent(m_idCurCell.row, m_idCurCell.col, GVN_SELCHANGED);
}
...
BOOL CGridCtrl::MouseOverRowResizeArea(CPoint& point)
{
    ...
    CCellID idCurCell = GetCellFromPt(point);
    CPoint start;
    if ( !GetCellOrigin( idCurCell, &start))
        return FALSE;

    int endy = start.y + GetRowHeight( idCurCell.row);

    if ( (point.y - start.y < m_nResizeCaptureRange && idCurCell.row != 0) ||
...
BOOL CGridCtrl::MouseOverColumnResizeArea(CPoint& point)
{
    ...
    int endx = start.x + GetColumnWidth( idCurCell.col);

    if ( (point.x - start.x < m_nResizeCaptureRange && idCurCell.col != 0) ||
    ...
    TRY 
    {
        ...
        if ( addedCols > 0)
        {
            ...
            for (col = startCol; col < nCols; col++)
                m_arColWidths[col] = m_cellFixedColDef.GetMbWidth();
                            ...
                            int nColumn  /* = -1 */)
{
    ...
    if ( m_idCurCell.col != -1 && nColumn < m_idCurCell.col)
        m_idCurCell.col++;
...
int CGridCtrl::InsertRow(LPCTSTR strHeading, int nRow /* = -1 */)
{
    ...
    if ( m_idCurCell.row != -1 && nRow < m_idCurCell.row)
        m_idCurCell.row++;
...
BOOL CGridCtrl::DeleteColumn( int nColumn)
{
    ...
    if ( nColumn == m_idCurCell.col)
        m_idCurCell.row = m_idCurCell.col = -1;
    else if ( nColumn < m_idCurCell.col)
        m_idCurCell.col--;
...
BOOL CGridCtrl::DeleteRow( int nRow)
{
    ...
    if ( nRow == m_idCurCell.row)
        m_idCurCell.row = m_idCurCell.col = -1;
    else if ( nRow < m_idCurCell.row)
        m_idCurCell.row--;
...
void CGridCtrl::AutoFill()
{
    ...
    SetColumnCount(rect.Width() / m_cellDefault.GetMbWidth() + 1);
...
void CGridCtrl::OnMouseMove(UINT /*nFlags*/, CPoint point)
{
        ...
        (m_bLMouseButtonDown && m_MouseMode == MOUSE_NOTHING))
    {
        ...
        if ( m_MouseMode == MOUSE_NOTHING)
        {
            ...
            if ( !GetVirtualMode() || m_bTitleTips)
            {
                ...
                // over it.
                idCurCell = GetCellFromPt(point);
                pCell = GetCell( idCurCell.row, idCurCell.col);
                if ( pCell)
                    pCell->OnMouseOver();
            }
            ...
            if ( m_bTitleTips)
            {
                ...
                if ( pCell)
                {
                    ...
                    if ( !m_bRMouseButtonDown
                        ...
                        && !pCell->IsEditing()
                        && GetCellRect( idCurCell.row, idCurCell.col, &TextRect)
                        && pCell->GetTipTextRect( &TextRect)
                        && GetCellRect( idCurCell.row, idCurCell.col, CellRect) )
                    {
    ...
    if ( m_bLMouseButtonDown)
    {
        ...
        case MOUSE_SELECT_CELLS:
            {
                CCellID idCurCell = GetCellFromPt(point);
                if ( !IsValid( idCurCell))
                    return;

                if ( idCurCell != GetFocusCell())
                {
                    OnSelecting( idCurCell);
                    ...
                    if(( idCurCell.row >= m_nFixedRows &&
                      idCurCell.col >= m_nFixedCols) ||
                      m_MouseMode==MOUSE_SELECT_COL ||
                      m_MouseMode==MOUSE_SELECT_ROW)
                    {
                        SetFocusCell( idCurCell);
                    }
                }
                break;
            }
...
void CGridCtrl::OnLButtonDown(UINT nFlags, CPoint point)
{
    ...
    else
    {
        if ( !IsValid(m_SelectionStartCell))
            m_SelectionStartCell = m_idCurCell;
    }
    ...
    if ( m_LeftClickDownCell == m_idCurCell && 
...
void CGridCtrl::OnLButtonUp(UINT nFlags, CPoint point)
{
    ...
    CPoint pointClickedRel;
    pointClickedRel = GetPointClicked( m_idCurCell.row, m_idCurCell.col, point);
    ...
    {
        OnEditCell(m_idCurCell.row, m_idCurCell.col, pointClickedRel, VK_LBUTTON);
    }
#ifndef GRIDCONTROL_NO_DRAGDROP
    ...
    {
        CGridCellBase* pCell = GetCell(m_idCurCell.row, m_idCurCell.col);
        if ( pCell)
            pCell->OnClick( GetPointClicked( m_idCurCell.row, m_idCurCell.col, point) );
...
void CGridCtrl::OnBeginPrinting(CDC *pDC, CPrintInfo *pInfo)
{
    ...
    CDC *pCurDC = GetDC();        // will have dimensions of the client area
    if ( !pCurDC) return;

    CSize PaperPixelsPerInch(pDC->GetDeviceCaps(LOGPIXELSX), pDC->GetDeviceCaps(LOGPIXELSY));
    CSize ScreenPixelsPerInch(pCurDC->GetDeviceCaps(LOGPIXELSX), pCurDC->GetDeviceCaps(LOGPIXELSY));
